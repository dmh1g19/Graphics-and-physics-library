 ...
 * rendering.cpp
 * 
 * This module implements the rendering functionalities for the application.
 * It includes functions for reading shader files, initializing buffers,
 * compiling and linking shaders, using specific shaders, and drawing shapes.
 * Additionally, it provides error-checking utilities for shader compilation
 * and OpenGL errors.
 *
 * The following initialization process is used:
 *
 ...
 * - readShaderFromFile: Reads and returns the content of a shader file.
 * - initBuffers: Initializes and binds the Vertex Array Object (VAO) and Vertex Buffer Object (VBO), sets vertex data, and configures vertex attributes.
 * - initShaders: Compiles vertex and fragment shaders, links them into a shader program, and returns the program ID.
 * - useSpecificShader: Uses a specific shader program.
 * - drawShape: Draws a shape using the specified shader program and vertex array object.
 * - checkCompileErrors: Checks and logs shader compilation and program linking errors.
 * - getGLErrorString: Returns a string representation of an OpenGL error code.
 *
